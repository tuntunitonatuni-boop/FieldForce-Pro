    ...
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden fixed inset-0">
      ...
      `}>
        ...
        <nav className="flex-1 px-4 space-y-2 relative z-10 overflow-y-auto no-scrollbar">
          ...
          {(isSuperAdmin || isBranchAdmin) && (
            <SidebarItem 
              ...
              label={isSuperAdmin ? "Control Center" : "Team Admin"}
              desc="Manage Staff"
            />
          )}
          <SidebarItem 
        ...
        <div className="p-6 mt-auto relative z-10">
          <div className="bg-slate-800/80 backdrop-blur-sm p-4 rounded-3xl border border-slate-700/50 flex items-center space-x-3 mb-4 group cursor-pointer hover:bg-slate-800 transition-colors">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center font-black text-lg text-white shadow-lg ring-2 ring-slate-700 group-hover:ring-blue-500 transition-all">
              {user.name.charAt(0)}
            </div>
          ...
          <button 
            onClick={onLogout}
            className="w-full py-4 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-2xl text-xs font-black transition-all flex items-center justify-center space-x-2 border border-red-500/20 active:scale-95"
          >
      ...
      <main className="flex-1 flex flex-col h-full w-full relative bg-slate-50">
        ...
        <header className="fixed top-0 left-0 right-0 h-16 md:h-20 bg-white/95 backdrop-blur-xl border-b border-slate-200/80 flex items-center justify-between px-4 md:px-10 z-[60] shadow-sm md:static md:shadow-none transition-all">
          ...
          <div className="md:hidden">
            <div className="w-9 h-9 rounded-full bg-slate-100 border-2 border-slate-200 flex items-center justify-center font-bold text-slate-600 text-sm shadow-sm">
              {user.name.charAt(0)}
            </div>
          </div>
        </header>
